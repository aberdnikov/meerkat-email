...
class Email {
    ...
    public static function connect($config = NULL) {
        ...
        ($config === NULL) and $config = Kohana::$config->load('meerkat/email');
    ...
    public static function send($to, $subject, $message, $html = FALSE) {
        if(Kohana::$config->load('meerkat/email.send_in_log')){
            $content = PHP_EOL."######################   EMAIL   ###########################".PHP_EOL.PHP_EOL;
            $content .= 'TO: '.$to.PHP_EOL.PHP_EOL;
            $content .= 'SUBJECT: '.$subject.PHP_EOL.PHP_EOL;
            $content .= $message.PHP_EOL.PHP_EOL;
            Kohana::$log->add(\Log::NOTICE,$content);
            return;
        }
        $from = Kohana::$config->load('meerkat/email.from');
